...
[cols="1"]
|===
Camera_gamma (stem:[\gamma _c]) : The characteristic of the image sensor.

Encoding_gamma (stem:[\gamma _e]) : The gamma of any transformation performed by the software writing the image file.

Decoding_gamma (stem:[\gamma _d]) : The gamma of any transformation performed by any software reading the image file.

LUT_gamma (stem:[\gamma _{lut}]) : The gamma of the frame buffer LUT, if present.


In addition, let's add a few other names:

File_gamma (stem:[\gamma _f]) : The gamma of the image in the file, relative to the original scene. This is:

stem:[\gamma _f = \gamma _c * \gamma _e]

Display_gamma (stem:[\gamma _{DS}}]) : The gamma of the “display system” downstream of the frame buffer. This is:

        display_gamma = LUT_gamma * CRT_gamma

stem:[\gamma _{dg} = \gamma _g * \gamma _{crt}]

Viewing_gamma (stem:[\gamma _v]) : The overall gamma that we want to obtain to produce pleasing images generally 1.0 to 1.25.
|===
