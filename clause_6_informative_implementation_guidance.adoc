...
1.  Camera_gamma (latexmath:[\gamma _c]): The characteristic of the image sensor.
2.  Encoding_gamma (latexmath:[\gamma _e]): The gamma of any transformation performed by the software writing the image file.
3.  Decoding_gamma (latexmath:[\gamma _d]): The gamma of any transformation performed by any software reading the image file.
4.  LUT_gamma (latexmath:[\gamma _{lut}]): The gamma of the frame buffer LUT, if present.

 In addition, let's add a few other names:

1.  File_gamma (latexmath:[\gamma _f]): The gamma of the image in the file, relative to the original scene, i.e. latexmath:[\gamma _f = \gamma _c \gamma _e]
2.  DS_gamma (latexmath:[\gamma _{DS}]): The gamma of the “display system” downstream of the frame buffer. In this context, the term display system encompasses everything after the frame buffer, that is latexmath:[\gamma _{DS} = \gamma _{lut} \gamma _{crt}]
3.  Viewing_gamma (latexmath:[\gamma _v]): The overall gamma that we want to obtain to produce pleasing images generally 1.0 to 1.25.
...
1.  The imagery file in the CDB is unmodified (i.e. those produced by the Adobe Photoshop at the DBGF). Note that as a result of viewing gamma of latexmath:[\gamma _v = 1.25], the file gamma ended up at latexmath:[\gamma _f = 1.25] at the DBGF. As a result, the CDB also has a file gamma of latexmath:[\gamma _f = 1.25]
2.  The IG performs all of its internal operations in a linear color space (i.e. the IG_gamma is latexmath:[\gamma _{IG} = 1.00])
3.  The simulator visual system produces an average scene brightness of approximately 6 ft-lamberts: under these viewing conditions, the viewing gamma is latexmath:[\gamma _v = 1.125].
4.  The measured gamma of the visual display system is latexmath:[\gamma _{crt} = 2.025]
5.  The content of the IG’s color look-up tables is adjusted to compensate for the gamma of the visual display system, i.e. it is loaded with latexmath:[\gamma _{lut} = \frac{1}{2.025}]
...
1.  The DBGF imagery application is Adobe Photoshop. The default color space profile used by Adobe Photoshop (i.e. the *.icm file) is the sRGB Color Space Profile which is defined by the sRGB standard to be a gamma of 2.2, therefore the Photoshop uses a latexmath:[\gamma _{lut} = \frac{1}{2.2}]
2.  The DBGF workstation is running Windows (therefore the O/S does not gammatize the imagery before sending it to the display, latexmath:[\gamma _{lut} = 1.25])
3.  The measure gamma of the DBGF workstation monitor is latexmath:[\gamma _{crt} = 2.2]
4.  The DBGF workstation is located in a dimly lit room, so the viewing gamma is in effect latexmath:[\gamma _v = 1.25]
